       ...
       SELECT system_item_nbr,
		...
		SELECT     system_item_nbr,
		 ...
		 GROUP BY   system_item_nbr,  visit_date 
             UNION ALL
		 SELECT     system_item_nbr,
                            0.01*sum(unit_qty)              AS day_sum_qty_1,
                            0.01*sum(retail_all)            AS day_sum_retail_1,
                            0.01*COUNT(DISTINCT visit_nbr, visit_date) AS day_sum_visit_1,
                            visit_date
                            FROM   (	
...
CREATE TABLE sams_us_dotcom_bucket_online_monthly AS
      ...
      FROM
	   ...
	   FROM  
	             ...
	             FROM (	
		          ...
		          FROM  	    (
				    SELECT     x.system_item_nbr,
					       ...
					       COALESCE(a.visit_date, '2016-07-14') AS visit_date,
					       '2016-07-14' as now_date,
					       '0.025' as lmda1,
					       '0.025' as lmda2,
					       '0.025' as lmda3
                                      FROM     
		...
		LEFT JOIN (
                    SELECT   system_item_nbr,
                       	     SUM (day_sum_visit) AS sum_visit_test,
			     SUM (day_sum_qty) AS sum_qty_test, 
			     SUM (day_sum_retail) AS sum_retail_test
                       	     FROM temp_test 
			     GROUP BY system_item_nbr
			     ) nn
	       ON (xx.system_item_nbr = nn.system_item_nbr)
		  ...
		  LEFT JOIN (
	       ...
	       JOIN
                   (SELECT catalog_item_id, bucket FROM jli21.sams_dotcom_item_cat_bucket WHERE ds ='2016-07-14' )bu
	       ON (xx.catalog_item_id = bu.catalog_item_id)

	       LEFT JOIN		      
                   (SELECT catalog_item_id, cat_parent, cat_child FROM jli21.sams_dotcom_item_cat_map WHERE ds ='2016-07-14' )bu_map
               ON (xx.catalog_item_id = bu_map.catalog_item_id and bu.bucket = bu_map.cat_parent)

	       LEFT JOIN
	           (SELECT athcpid, COUNT (*) AS impression FROM impression_1_split
		    GROUP BY athcpid ) impression
	       ON (xx.catalog_item_id = impression.athcpid)

	       LEFT JOIN
                   (SELECT athcpid, COUNT (*) AS click FROM click_1
		    GROUP BY athcpid ) click
               ON (xx.catalog_item_id = click.athcpid)
	 )f1	      	                 
	       CROSS JOIN (
	       ...
	       CROSS JOIN (
	       	     SELECT  SUM (day_sum_visit) AS sum_sum_visit_test
                             FROM temp_test
                                                            )test_peroid 	      							  		
;                                       
